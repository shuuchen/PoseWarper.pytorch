...
class Conv(nn.Module):
	def __init__(self, in_ch, out_ch, kernel_size=3, stride=1, relued=True):
		...
		self.conv_bn = nn.Sequential(
				...
				#nn.GroupNorm(out_ch // CHANNELS_PER_GROUP, out_ch))
		self.relu = nn.ReLU()
		self.relued = relued
...
class HRNet_Warping(nn.Module):
	def __init__(self, in_ch, mid_ch, out_ch, num_stage=4, warping=True):
		super().__init__()
	...
	def forward(self, x):
		...
		# warping
		if self.warping:
			true_batch_size = x_list[0].size(0) // 2

			ref_x = x[:true_batch_size,:,:,:]
			sup_x = x[true_batch_size:,:,:,:]
